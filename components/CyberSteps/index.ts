import { arrowDownIcon, arrowIcon } from '../../assets/svgicons';
...
function injectStyles() {
  ...
  .cyber-steps-section .cta-button { background: white; color: #005DFF; border: none; padding: 22px 60px; border-radius: 10px; font-size: 1.125rem; font-weight: 700; cursor: pointer; box-shadow: 0 2px 8px rgba(0,0,0,0.1); min-width: 242px; display: flex; align-items: center; justify-content: center; gap: 12px; margin: 0 auto; text-align: center; }
  .cyber-steps-section .cta-button:hover { transform: translateY(-2px); box-shadow: 0 4px 16px rgba(0,0,0,0.15); border-radius: 0; padding: 18px 30px; }
  .cyber-steps-section .cta-arrow { display: none; align-items: center; justify-content: center; opacity: 0; transition: opacity 0.8s linear 0.18s; }
  .cyber-steps-section .cta-button:hover .cta-arrow { display: flex; opacity: 1; transition-delay: 0.18s; }
  .cyber-steps-section .cta-label { display: flex; align-items: center; justify-content: center; width: 100%; }
...
export function mountCyberSteps(target: HTMLElement, configArg?: Partial<CyberStepsConfig>) {
  ...
  function render() {
    ...
    ctaBtn.innerHTML = `<span class="cta-arrow">${arrowIcon}</span><span class="cta-label">${config.cta?.label || 'GET A QUOTE'}</span>`;
